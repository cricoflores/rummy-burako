...
// Prepara inputs (si necesitás habilitarlos, aquí es)
...
// Al tocar “Terminar Turno”: detengo y avanzo + reinicio timer
endTurnBtn.addEventListener('click', e => {
  clearInterval(timerInterval);

  // Si querés, podés guardar aquí 0 por defecto:
  // scores[currentPlayer-1][currentRound-1] = 0;

  // Avanzo jugador/ronda
...
// Guardar puntos para la ronda actual
pointsForm.addEventListener('submit', e => {
  e.preventDefault();
  // Leemos todos los inputs de esta ronda
  for (let i = 1; i <= numPlayers; i++) {
    const val = +document.getElementById(`input-player-${i}`).value || 0;
    scores[i-1][currentRound-1] = val;
  }
  alert('Puntos guardados para la ronda ' + currentRound);
});

// Inicialización de cada turno
function initTurn() {
  renderHeader();
  renderTable();
  renderInputs();
